...
fn shutdown_request_replies_then_sets_flag() {
    ...
    assert!(matches!(dispatch(&env, &state), Outcome::Reply(_)));
...
fn shutdown_notification_sets_flag_without_reply() {
    ...
    assert!(matches!(dispatch(&env, &state), Outcome::NoReply));
    assert!(state.is_shutting_down());
}
...
fn dispatch_tolerates_error_only_envelopes() {
    ...
    weird.method = None;
    assert!(matches!(dispatch(&weird, &state), Outcome::NoReply));
}
